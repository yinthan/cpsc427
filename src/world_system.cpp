...
bool WorldSystem::step(float elapsed_ms_since_last_update) {
	...
	if (registry.softShells.components.size() <= MAX_FISH && next_fish_spawn < 0.f) {
		...
		motion.position =
			vec2(screen_width -200.f, 
				 50.f + uniform_dist(rng) * (screen_height - 100.f));
		motion.velocity = vec2(-400.f, 0.f);
	}
	...
	for (Entity entity : registry.lightUps.entities) {
		...
		counter.counter_ms -= elapsed_ms_since_last_update;
		// dim the salmon once the lightUp timer expired
		if (counter.counter_ms < 0) {
		...
		if (registry.players.has(entity)) {
			...
			if (registry.hardShells.has(entity_other)) {
				...
				if (!registry.deathTimers.has(entity)) {
					...
					// registry.colors.get(entity).r = 1.f;
					registry.colors.get(entity).g = 0.f;
					registry.colors.get(entity).b = 0.f;
				}
			}
...
void WorldSystem::on_key(int key, int, int action, int mod) {
	...
	Motion& m_salmon = registry.motions.get(player_salmon);
	if (!registry.deathTimers.has(player_salmon)) {
		switch (key) {
		case GLFW_KEY_RIGHT:
			if (action == GLFW_PRESS)
				m_salmon.velocity = { 200.f , 0.f };
			if (action == GLFW_RELEASE)
				m_salmon.velocity = { 0.f , 0.f };
			break;
		case GLFW_KEY_LEFT:
			if (action == GLFW_PRESS)
				m_salmon.velocity = { -200.f , 0.f };
...
void WorldSystem::on_mouse_move(vec2 mouse_position) {
	...
	Motion& m_salmon = registry.motions.get(player_salmon);
	float x = m_salmon.position.x;
	float y = m_salmon.position.y;
	float angle = atan((mouse_position.y - y) / (mouse_position.x - x));
	m_salmon.angle = angle;
